...
if (CC_EDITOR) {
  if (!Editor.CocosService_gmeDemo) {
    ...
    Editor.log("这是一个简单的 GME 示例 Demo，通过本示例您可以快速了解如何使用 GME 来实现游戏语音功能！");
  }
  if (appid === '' || key === '') {
    Editor.log("您需要首先从右侧的服务面板开启 腾讯云GME 服务，然后从 腾讯云GME 服务面板前往腾讯云控制台获取 SDK 初始化参数并填写到示例代码中的相关位置，以使 Demo 能够运行。");
...
cc.Class({
  ...
  initGME: function () {
    ...
    this.printCode(`tencentGME && tencentGME.init('${appid}', '${key}', '${openid}');`);
  ...
  btnRemoteStream: function () {
    ...
    this.printCode(`tencentGME && tencentGME.enableSpeaker(${this.speakerEnabled});`);
  },
